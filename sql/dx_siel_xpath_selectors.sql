  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','detail',d,'discount_type',
       '//*[@id="slot-list-container"]/div/div[2]//*[contains(text(),"Hot Deal") or contains(text(),"Hot deal") or contains(text(),"Super Deals") or contains(text(),"Saver Deal") or contains(text(),"SALE PE SALE") or contains(text(),"Lowest Price Live") or contains(text(),"Lowest price since launch") or contains(text(),"Limited time") or contains(text(),"Limited Time") or contains(text(),"Big Saving") or contains(text(),"Coupon") or contains(text(),"Special Price") or contains(text(),"Deal of") or contains(text(),"Early bird") or contains(text(),"Early Bird")]',
       NULL,
       '사용자 검수 (5/10 #6): detail page 단어 list + 메인 product 영역 (slot-list-container/div/div[2]) 만 매치. 이전 selector page 전체 → 추천 카드 / similar products 의 deal text 도 매치 (REF detail evidence 12건 noise — 본 product 미해당). 단어 list (Saver Deal/SALE PE SALE/Lowest Price Live/Lowest price since launch). detail.py 분기에서 multi-match 합침 + Bank Offer/재고/Exchange 영역 필터.'),
      ('Flipkart','detail',d,'sku_popularity',
       '//*[@id="slot-list-container"]/div/div[2]//*[contains(text(),"Bestseller") or contains(text(),"BESTSELLER") or contains(text(),"Flipkart''s Choice") or contains(text(),"Flipkart Choice")] | //*[@id="slot-list-container"]/div/div[2]//img[contains(@src,"/fa_")]',
       NULL,
       'detail page Bestseller marker. 5/10 #6 사용자 검수: 메인 product 영역 (slot-list-container/div/div[2]) 만 매치. 이전 selector page 전체 → 추천 카드 의 Bestseller marker 도 매치 (REF detail evidence 6건 noise — 본 product 미해당). 메인 영역 만 + cls "BESTSELLER" / "Bestseller" / "Flipkart Choice" / fa_*.png img.')
    ON CONFLICT (site_account, page_type, domain, data_field) DO UPDATE SET
      xpath_primary  = EXCLUDED.xpath_primary,
      fallback_xpath = EXCLUDED.fallback_xpath,
      notes          = EXCLUDED.notes,
