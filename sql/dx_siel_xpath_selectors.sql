  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','detail',d,'discount_type',
       '//*[contains(text(),"Hot Deal") or contains(text(),"Hot deal") or contains(text(),"Super Deals") or contains(text(),"Saver Deal") or contains(text(),"SALE PE SALE") or contains(text(),"Limited time") or contains(text(),"Limited Time") or contains(text(),"Big Saving") or contains(text(),"Coupon") or contains(text(),"Special Price") or contains(text(),"Deal of") or contains(text(),"Trending") or contains(text(),"Early bird") or contains(text(),"Early Bird")]',
       NULL,
       '사용자 검수 (5/9 #2): detail page layout 변형 다수 — cls "HZ0E6r" 미사용 page 존재 (TV detail 102/102 NULL 결함). 단어 list 로 revert. Bank Offer/Bank offer 제외 (사용자 정책) + Saver Deal/SALE PE SALE 추가. detail.py 분기에서 multi-match 합침 + Bank Offer/재고/Exchange 영역 필터 적용. main 의 cls "HZ0E6r" selector 와 형식 다름 — Lowest Price Live deal type 여부 사용자 검토 대기.'),
