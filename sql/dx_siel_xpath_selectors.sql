  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','detail',d,'detailed_review_content',
       '//span[@class="css-1jxf684"]/span[contains(@class,"css-1jxf684") and string-length(normalize-space(text()))>0 and not(normalize-space(text())="more")]',
       NULL,
       '리뷰 페이지 navigate 후. 5/10 #4 사용자 evidence (review body dom): outer <span class="css-1jxf684"> (cls 단독, exact) → inner <span class="css-1jxf684 r-1et8rh5 r-1b43r93"> (multi-class) = review body text. 이전 selector @class="css-1jxf684" exact 만 매치 → "Positive feedback" / "Negative feedback" sentiment summary 만 잡음. 신: outer/inner nested 패턴 — outer 단독 cls, inner contains. sentiment span (직접 자식, nested 없음) 차단. fallback: primary 매치 X 시 NULL (자연 — 본 product review 적은 경우)'),
