  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','detail',d,'detailed_review_content',
       '//span[contains(@class,"css-1jxf684") and string-length(normalize-space(text()))>0 and not(normalize-space(text())="more") and not(normalize-space(text())="Positive feedback") and not(normalize-space(text())="Negative feedback")]',
       NULL,
       '리뷰 페이지 navigate 후. 5/10 #5 사용자 evidence — review body dom 가 driver state 별 layout 변형: (1) detail → click navigate 흐름 시 nested pattern (outer/inner span), (2) review URL 직접 navigate 시 single span. 이전 selector outer/inner nested 패턴 만 → single span layout 매치 X 결함. 신 selector — cls "css-1jxf684" contains + length>0 + sentiment label exact 제외 ("Positive feedback"/"Negative feedback") + "more" 제외. single span (text 직접) + nested inner span (text 직접) 둘 다 매치, nested outer (text 빈) 자동 제외 (length=0). count_of_reviews 만큼 수집 (최대 20) — fpkt/detail.py 가 &page=2,3 누적'),
      ...
      ('Flipkart','detail',d,'discount_type',
       '//*[contains(text(),"Hot Deal") or contains(text(),"Hot deal") or contains(text(),"Super Deals") or contains(text(),"Saver Deal") or contains(text(),"SALE PE SALE") or contains(text(),"Lowest Price Live") or contains(text(),"Lowest price since launch") or contains(text(),"Limited time") or contains(text(),"Limited Time") or contains(text(),"Big Saving") or contains(text(),"Coupon") or contains(text(),"Special Price") or contains(text(),"Deal of") or contains(text(),"Early bird") or contains(text(),"Early Bird")]',
       NULL,
