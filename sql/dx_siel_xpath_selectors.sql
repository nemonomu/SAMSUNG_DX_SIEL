  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Amazon','main',d,'discount_type',
       './/span[contains(@class,"s-coupon-clipped") or contains(@class,"a-badge-text")][text()!=""]',
       './/*[contains(@id,"DEAL_") and contains(@id,"-label")]//span',
       'Limited time deal / Coupon 등'),
...
VALUES
  ...
  ('Amazon','detail','hhp','fastest_delivery',
   '//*[@id="mir-layout-DELIVERY_BLOCK-slot-SECONDARY_DELIVERY_MESSAGE_LARGE"]//span[1]',
   NULL,
   '앞 "Or" / 끝 "Details" 후처리에서 제거'),
  ...
  ('Amazon','detail','hhp','sku',
   '//table[contains(@id,"productDetails_detailBullets") or contains(@id,"productDetails_techSpec") or contains(@id,"productDetails_expanderTables")]//tr[.//th[contains(text(),"ASIN")]]/td',
...
VALUES
  ...
  ('Amazon','detail','tv','retailer_sku_name_similar',
   '//*[@id="anonCarousel2"]//li//div[contains(@class,"a-truncate-full")]',
   NULL, NULL),
  ...
  ('Amazon','detail','tv','summarized_review_content',
   '//*[@id="reviewsMedley"]//div[contains(@id,"review-summary")]//span', NULL, NULL),
...
VALUES
  ...
  ('Amazon','detail','ref','fastest_delivery',
   '//*[@id="mir-layout-DELIVERY_BLOCK-slot-SECONDARY_DELIVERY_MESSAGE_LARGE"]//span[1]', NULL, NULL),
  ('Amazon','detail','ref','inventory_status',
   '//*[@id="availability"]//span', NULL, NULL),
  ('Amazon','detail','ref','retailer_sku_name_similar',
   '//*[@id="anonCarousel2"]//li//div[contains(@class,"a-truncate-full")]', NULL, NULL),
  ('Amazon','detail','ref','star_rating',
   '//*[@id="acrPopover"]//span[@class="a-size-base a-color-base"]', NULL, NULL),
  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ('Flipkart','detail',d,'expand_specifications',
       '//button[normalize-space(text())="Read More" or contains(text(),"Specifications")] | //div[contains(text(),"All Specifications")]/ancestor::button',
       '//*[contains(text(),"All details")]/ancestor::div[1]',
       'Specifications / All details 펼치기 — robust click 적용'),
      ('Flipkart','detail',d,'click_show_all_reviews',
       '//div[normalize-space(text())="All reviews" or contains(text(),"Show all reviews")] | //a[contains(@href,"/product-reviews/") and not(contains(@href,"buynow"))]',
       '//div[contains(text(),"All ") and contains(text(),"reviews")]',
       '리뷰 페이지 진입 — Buy now 회피 주의'),
      ('Flipkart','detail',d,'star_rating',
       '//div[contains(@class,"XQDdHH") and string-length(normalize-space(text()))<=4][1]',
       '//div[@id and contains(@id,"productRating_")]/div',
       '"3.9" 등 짧은 숫자만'),
      ('Flipkart','detail',d,'count_of_star_ratings',
       '//span[contains(text(),"Ratings")]/preceding-sibling::span | //span[contains(text(),"Ratings")]/parent::*//text()[1]',
       '//*[contains(text(),"Ratings") and contains(text(),"&")]',
       '"39,132 Ratings" 앞 숫자만'),
      ('Flipkart','detail',d,'count_of_reviews',
       '//span[contains(text(),"Reviews")]/preceding-sibling::span',
       '//*[contains(text(),"Reviews") and contains(text(),"&")]',
       '"2,391 Reviews" 앞 숫자만 — code 가 1 이상일 때만 review 추출'),
      ('Flipkart','detail',d,'delivery_availability',
       '//div[contains(text(),"Delivery") and (contains(text(),"Free") or contains(text(),"₹"))]',
       '//div[contains(text(),"delivery") and contains(text(),"by")]', NULL),
      ('Flipkart','detail',d,'detailed_review_content',
       '//div[contains(@class,"ZmyHeo") or contains(@class,"t-ZTKy")]//div[string-length(normalize-space(text()))>50]',
       '//div[@data-tkid]//div[string-length(normalize-space(text()))>50]',
       '리뷰 본문 다중 — Show all reviews 클릭 후 review page 에서 추출 (max 20)'),
      ('Flipkart','detail',d,'retailer_sku_name_similar',
       '//div[contains(text(),"Similar")]/ancestor::section//a[contains(@href,"/p/")]//div[string-length(normalize-space(text()))>10][1]',
       '//div[@class[contains(.,"similar")]]//a[contains(@href,"/p/")]/@title',
       'Similar Products 다중 추출'),
      ('Flipkart','detail',d,'sku',
       '//td[normalize-space(text())="Model Name"]/following-sibling::td[1] | //*[normalize-space(text())="Model Name"]/following-sibling::*[1]',
       '//tr[.//td[contains(text(),"Model")]]/td[2]',
       'Specifications > General > Model Name (Specifications 클릭 후)');
  END LOOP;
END $$;
...
VALUES
  ('Flipkart','detail','hhp','final_sku_price',
   '//div[starts-with(normalize-space(text()),"₹") and not(contains(@style,"strike")) and not(contains(@class,"strike"))][1]',
   '//div[contains(@class,"_30jeq3") or contains(@class,"Nx9bqj")]',
   'HHP: detail 에서 가격 수집'),
  ('Flipkart','detail','hhp','original_sku_price',
   '//div[contains(@class,"strike") or contains(@style,"line-through")][starts-with(normalize-space(text()),"₹")]',
  ...
  ('Flipkart','detail','hhp','trade_in',
   '//div[contains(text(),"Exchange offer") or contains(text(),"Up to ₹")][1]',
   '//*[contains(text(),"Exchange")]/ancestor::div[1]',
   '두 xpath 결과 공백 1개 두고 합치기 — 후처리 필요'),
  ('Flipkart','detail','hhp','hhp_storage',
   '//ul/li[contains(text(),"GB ROM") or contains(text(),"GB RAM") or (contains(text(),"GB") and contains(text(),"ROM"))][1]',
   '//td[normalize-space(text())="Internal Storage"]/following-sibling::td[1]',
   'Product highlights ROM 또는 Spec table'),
  ('Flipkart','detail','hhp','hhp_color',
   '//td[normalize-space(text())="Color"]/following-sibling::td[1]',
   '//tr[.//td[contains(text(),"Colour") or contains(text(),"Color")]]/td[2]',
   'Spec 테이블에서');
