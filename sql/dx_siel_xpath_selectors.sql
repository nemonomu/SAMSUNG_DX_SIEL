  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','main',d,'discount_type',
       ...
       NULL,
       '사용자 정책 (5/9): cls "HZ0E6r Rm9_cy" deal badge innermost div 매치 (단어 list 폐기). 신규 deal type (Summer Deal / Anniversary Deal 등) 자동 수집. listing.py 의 discount_type 분기에서 Bank Offer 제외 + Exchange offer 영역 ("Upto" / "₹X" / "on Exchange") 제외 + 길이 < 50 + ", " 합침. 4 도메인 main/detail 동일 cls.'),
  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','detail',d,'discount_type',
       '//div[contains(@class,"HZ0E6r")]',
       NULL,
       '사용자 console 검증 (5/9): detail page deal badge 도 main 과 동일 cls "HZ0E6r Rm9_cy" innermost div. detail.py 의 discount_type 분기에서 Bank Offer 제외 + Exchange offer 영역 제외 + 길이 < 50 + ", " 합침. main fallback 시 형식 일관'),
