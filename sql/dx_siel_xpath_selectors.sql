  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Amazon','main',d,'final_sku_price',
       './/span[@class="a-price" and @data-a-color="base"]//span[@class="a-offscreen"] | .//span[@aria-label="Currently unavailable."]/span | .//span[normalize-space(text())="Currently unavailable."] | .//*[@id="fod-cx-message-with-learn-more"]/span[1] | .//span[normalize-space(text())="No featured offers available"] | .//span[normalize-space(text())="See price in cart"] | .//span[normalize-space(text())="Temporarily out of stock"] | .//span[normalize-space(text())="Price higher than typical"]',
       './/span[contains(@class,"a-price") and not(@data-a-strike)]//span[@class="a-offscreen"]',
       '가격 매치 우선 (DOM 순서). 가격 부재 시 fallback union: (1) "Currently unavailable." (aria-label 또는 text fullmatch), (2) "No featured offers available" (fod-cx-message-with-learn-more id 또는 text fullmatch), (3) "See price in cart" / "Temporarily out of stock" / "Price higher than typical" (text fullmatch). variant chooser ("See options") 카드는 valid null — detail 에서 채움'),
      ...
      ('Amazon','main',d,'sku_popularity',
       ...
       'Amazon Choice / Best Seller 배지. 5/10 fix: 옛 contains(text(),"Best Seller") 는 case-sensitive — 실제 dom 의 aria-label="Best seller" (소문자 s) 누락 root cause (REF main 검증). BEST_SELLER wrapper id (case 무관, Amazon 표준 anchor) 사용으로 fix. 4 product 공통.'),
...
VALUES
  ...
  ('Amazon','detail','hhp','retailer_sku_name_similar',
   '//div[@aria-labelledby="Customers who viewed this item also viewed"]//li[not(contains(@class,"a-carousel-card-empty"))]//a[contains(@href,"/dp/")]//div[contains(@class,"p13n-sc-truncate-fallback")]',
   '//*[contains(@id,"sims-fbt")]//div[contains(@class,"a-truncate-full")]',
   'sims-simsContainer carousel 한정 (aria-labelledby="Customers who viewed this item also viewed") + dp link + p13n-sc-truncate-fallback div text. carousel 첫 page 6~7 SKU. 섹션 자체 없는 page → NULL 정상. 5/10 정정: 옛 broad anonCarousel union 은 Bank Offer / Best Sellers 등 noise carousel 까지 매치 — sims carousel scope 한정으로 root cause 차단 (Bank Offer carousel 자체가 매치 X 이므로 historical "dp link 부재" 회귀 위험 없음)'),
  ...
  ('Amazon','detail','hhp','hhp_storage',
   '//*[@id="productOverview_feature_div"]//tr[contains(@class,"po-memory_storage_capacity")]/td[2]//span | //*[@id="productDetails_expanderTables_depthLeftSections"]//table[contains(@class,"prodDetTable")]//tr[.//th[contains(normalize-space(.),"Memory Storage Capacity") or contains(normalize-space(.),"Internal Memory")]]/td',
   '//table//tr[.//*[self::th or self::td][contains(normalize-space(.),"Memory Storage Capacity") or contains(normalize-space(.),"Internal Memory") or contains(normalize-space(.),"Storage Capacity")]]/td[last()]',
   'e.g. "64 GB". Product Overview po-memory_storage_capacity 우선, Additional details prodDetTable fallback.'),
...
VALUES
  ...
  ('Amazon','detail','tv','retailer_sku_name_similar',
   '//div[@aria-labelledby="Customers who viewed this item also viewed"]//li[not(contains(@class,"a-carousel-card-empty"))]//a[contains(@href,"/dp/")]//div[contains(@class,"p13n-sc-truncate-fallback")]',
   '//*[contains(@id,"sims-fbt")]//div[contains(@class,"a-truncate-full")]',
   'sims-simsContainer carousel 한정 (aria-labelledby="Customers who viewed this item also viewed") + dp link + p13n-sc-truncate-fallback div text. carousel 첫 page 6~7 SKU (화살표 click navigation X). 섹션 자체 없는 page → NULL 정상'),
  ...
  ('Amazon','detail','tv','sku',
   ...
   '//table//tr[.//th[normalize-space()="Model Number" or normalize-space()="Part Number" or normalize-space()="Item Model Number" or normalize-space()="Item model number" or normalize-space()="Item Part Number" or normalize-space()="Item part number" or normalize-space()="Model Name"]]/td',
   'TV: Manufacturer Part Number primary. fallback union (Model Number | Part Number | Item Model Number | Item model number | Item Part Number | Item part number | Model Name) — 5/8 진단 (SANSUI Model Number, Midea Part Number) + 5/9 진단 (LDY spam ND-95/IKERIA005/MWM-96055 — Item part number 라벨) + 5/30 추가 (15건 sku null 시리즈 B0FDL/B0FJL — Model Name 만 있는 케이스)'),
  ('Amazon','detail','tv','sku_assurance',
   '//*[@id="freeShippingPriceBadging_feature_div"]//span[contains(@class,"a-icon-text-fba")] | //*[@id="freeShippingPriceBadging_feature_div"]//i//span',
   '//*[contains(@id,"shippedBy") or contains(@id,"merchant-info")]//span',
   '"Fulfilled" → 후처리에서 "Amazon Fulfilled" 로 저장'),
  ('Amazon','detail','tv','screen_size',
   '//*[@id="poExpander"]//table//tr[.//td[contains(text(),"Screen Size")]]/td[2] | //table[contains(@class,"prodDetTable")]//tr[.//th[contains(text(),"Screen Size")]]/td[contains(@class,"prodDetAttrValue")]',
   NULL,
   'TV screen_size — content 검증 (Screen Size text) + main spec table 한정. 5/10 정정: Union 3 (poExpander positional) 제거 — 첫 행 가정 결함 (다른 product 에서 Brand/Display Type 등 잘못된 값 매치 위험). Union 2 의 broad //table 도 prodDetTable class + prodDetAttrValue td 한정 (variant carousel/comparison table 차단).'),
...
VALUES
  ...
  ('Amazon','detail','ref','sku',
   '//table//tr[.//th[contains(text(),"Manufacturer") and contains(text(),"Part Number")]]/td',
   '//table//tr[.//th[normalize-space()="Model Number" or normalize-space()="Part Number" or normalize-space()="Item Model Number" or normalize-space()="Item model number" or normalize-space()="Item Part Number" or normalize-space()="Item part number" or normalize-space()="Model Name"]]/td',
   'REF: Manufacturer Part Number primary. fallback union (Model Number | Part Number | Item Model Number | Item model number | Item Part Number | Item part number | Model Name) — 5/8 진단 (Midea Part Number) + 5/9 진단 (LDY spam Item part number 라벨 — REF 도 같은 패턴 가능성) + 5/30 일관성 (LDY/TV 와 동일 union, 미래 방어)'),
...
VALUES
  ...
  ('Amazon','detail','ldy','detailed_review_content',
   ...
   '//div[contains(@id,"customer_review-")]//span[@data-hook="review-body"]',
   'LDY 전용 — Amazon laundry page 의 review markup 이 시점에 따라 변동: reviewText (REF 패턴) 또는 review-collapsed/review-body (HHP 패턴). primary 두 union 으로 cover. 5/10 fallback narrow: 옛 reviewTextContainer//span 부분 제거 (REF 와 같은 noise risk — "Read moreRead less"/빈 span 매치). customer_review-{ID} review-body 부분만 보존 (정확). 메모: feedback_domain_branching_pattern.md'),
  ('Amazon','detail','ldy','sku',
   '//table//tr[.//th[contains(text(),"Manufacturer") and contains(text(),"Part Number")]]/td',
   '//table//tr[.//th[normalize-space()="Model Number" or normalize-space()="Part Number" or normalize-space()="Item Model Number" or normalize-space()="Item model number" or normalize-space()="Item Part Number" or normalize-space()="Item part number" or normalize-space()="Model Name"]]/td',
   'LDY: Manufacturer Part Number primary. fallback union (Model Number | Part Number | Item Model Number | Item model number | Item Part Number | Item part number | Model Name) — 5/8 진단 + 5/9 spam 진단 (3 표본 ND-95/IKERIA005/MWM-96055 모두 "Item part number" 라벨 사용). 5/29 추가: B0CDM19VXH (LG T70AJMB1Z) / B0B9SG9V9H (Whirlpool ACE 7.5 ...) 등 Model Name 만 있는 케이스 → Model Name 도 union'),
  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Amazon','detail',d,'final_sku_price',
       ...
       '(//div[@id="corePriceDisplay_desktop_feature_div"]//span[@class="a-price" and @data-a-color="base"]//span[@class="a-offscreen"])[1] | (//div[@id="centerCol"]//span[@class="a-price" and @data-a-color="base"]//span[@class="a-offscreen"])[1] | (//*[@id="outOfStock"]//span[contains(@class,"a-color-price")])[1] | (//*[@id="availability"]//span[contains(@class,"primary-availability-message")])[1] | (//*[@id="fod-cx-message-with-learn-more"]/span)[1] | (//span[normalize-space(text())="See price in cart"])[1] | (//span[normalize-space(text())="Temporarily out of stock"])[1] | (//span[normalize-space(text())="Price higher than typical"])[1]',
       '신 layout: apex-pricetopay-accessibility-label text (₹X.00 with N percent savings) — siel_log.parse_amzn_apex_price 로 ₹X 만 추출. fallback union: 구 a-offscreen layout | outOfStock div ("Currently unavailable.") | availability div primary-availability-message (신 unavailable layout — 5/11 batch 9건 NULL evidence) | fod-cx-message ("No featured offers available") | "See price in cart" / "Temporarily out of stock" / "Price higher than typical" (text fullmatch). variant carousel 회피 위해 corePriceDisplay scope [1] 한정'),
  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','main',d,'sku_popularity',
       './/a[contains(@href,"spotlightTagId=default_BestsellerId") or contains(@href,"spotlightTagId=default_TrendingId")] | .//img[contains(@src,"/fa_")] | .//*[contains(text(),"Flipkart''s Choice") or contains(text(),"Flipkart Choice")]',
       NULL,
       'Bestseller (anchor href spotlightTagId=default_BestsellerId) / Trending (anchor href spotlightTagId=default_TrendingId_<category>) / Flipkart Assured (img /fa_*.png) / Flipkart''s Choice (text). 코드가 attr/text 검사 후 라벨 합침. Trending = main page Bestseller 동일 위치 popularity marker (5/10 사용자 검수 — discount_type 아님).'),
...
BEGIN
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      -- count_of_reviews is still main-page only; detail can safely recover top product star_rating/count_of_star_ratings from the rating summary link.
      ('Flipkart','detail',d,'star_rating',
       '(//*[@id="slot-list-container"]/div/div[2]//a[contains(@href,"/ratings-reviews-details-page")]//div[contains(@class,"css-146c3p1") and contains(@style,"inter_bold") and (string-length(normalize-space(text()))=1 or (string-length(normalize-space(text()))=3 and substring(normalize-space(text()),2,1)=".")) and number(normalize-space(text()))=number(normalize-space(text())) and number(normalize-space(text()))>=1 and number(normalize-space(text()))<=5])[1]',
       NULL,
       'Top product rating only. Scope to /ratings-reviews-details-page summary link to avoid sponsored/similar product rating noise on no-rating products.'),
      ('Flipkart','detail',d,'count_of_star_ratings',
       '(//*[@id="slot-list-container"]/div/div[2]//a[contains(@href,"/ratings-reviews-details-page")]//div[contains(@class,"css-146c3p1") and contains(normalize-space(.),"|")])[1]',
       NULL,
       'Top product rating summary count, e.g. "| 18,824". parse_count_of_ratings strips the pipe. NULL when no top rating summary exists.'),
      ...
      ('Flipkart','detail',d,'detailed_review_content',
       '//span[contains(@class,"css-1jxf684") and string-length(normalize-space(text()))>0 and not(normalize-space(text())="more") and not(normalize-space(text())="Positive feedback") and not(normalize-space(text())="Negative feedback") and not(substring(normalize-space(text()),string-length(normalize-space(text())))="%")]',
       NULL,
       '리뷰 페이지 navigate 후. 5/10 #6 사용자 검수 (REF review): sentiment percentage element ("94%" / "6%") 도 cls "css-1jxf684" 매치 noise. 이전 selector exact label 만 차단 ("Positive feedback"/"Negative feedback") — percentage 미차단. 신: text 끝 character "%" 제외 추가. review body 가 "%" 끝 가능성 매우 낮음 (보통 "." 또는 space). single span + nested inner span 둘 다 매치 + sentiment label exact + percentage 차단. count_of_reviews 만큼 수집 (최대 20)'),
      ...
      ('Flipkart','detail',d,'discount_type',
       '//*[@id="slot-list-container"]/div/div[2]/div/div/div/div[1]//*[contains(text(),"Hot Deal") or contains(text(),"Hot deal") or contains(text(),"Super Deals") or contains(text(),"Saver Deal") or contains(text(),"SALE PE SALE") or contains(text(),"Lowest Price Live") or contains(text(),"Lowest price since launch") or contains(text(),"Limited time") or contains(text(),"Limited Time") or contains(text(),"Big Saving") or contains(text(),"Coupon") or contains(text(),"Special Price") or contains(text(),"Deal of") or contains(text(),"Early bird") or contains(text(),"Early Bird")]',
       NULL,
       '사용자 검수 (5/10 #7): detail page 메인 product 영역 = slot-list-container/div/div[2]/div/div/div/div[1]. 이전 path .../div[2]//* 가 div[1] (메인) + div[2] (추천 카드) 둘 다 매치 → REF detail 추천 카드 의 Saver Deal/Lowest Price Live noise 12건. evidence 사용자: 메인 product final_sku_price = .../div[2]/.../div[1]/..., 추천 Bestseller/Saver Deal = .../div[2]/.../div[2]/.... 단어 list (Saver Deal/SALE PE SALE/Lowest Price Live/Lowest price since launch). detail.py 분기에서 multi-match 합침.'),
      ('Flipkart','detail',d,'sku_popularity',
       '//*[@id="slot-list-container"]/div/div[2]/div/div/div/div[1]//*[contains(text(),"Bestseller") or contains(text(),"BESTSELLER") or contains(text(),"Flipkart''s Choice") or contains(text(),"Flipkart Choice")] | //*[@id="slot-list-container"]/div/div[2]/div/div/div/div[1]//img[contains(@src,"/fa_")]',
       NULL,
       'detail page Bestseller marker. 5/10 #7 사용자 검수: 메인 product 영역 (.../div[2]/div/div/div/div[1]) 만 매치. 이전 path .../div[2]//* 가 추천 카드 영역 .../div[2]/.../div[2] 도 매치 → REF detail 6건 noise. cls "BESTSELLER" / "Bestseller" / "Flipkart Choice" / fa_*.png img union.')
    ON CONFLICT (site_account, page_type, domain, data_field) DO UPDATE SET
      xpath_primary  = EXCLUDED.xpath_primary,
      fallback_xpath = EXCLUDED.fallback_xpath,
      notes          = EXCLUDED.notes,
...
VALUES
  ...
  ('Flipkart','detail','hhp','trade_in',
   ...
   NULL,
   'Exchange offer 의 "Up to ₹X" 또는 "Off" 표지. 5/10 #7 사용자 검수: fallback "//*[contains(text(),"Exchange")]/ancestor::div[1]" 가 "Exchange offer / Not available at this Pincode" 라벨 noise 매치 결함 → 제거. primary 매치 X 시 NULL.'),
