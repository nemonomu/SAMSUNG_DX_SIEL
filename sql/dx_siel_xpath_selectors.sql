  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Amazon','detail',d,'final_sku_price',
       ...
       '신 layout: apex-pricetopay-accessibility-label text (₹X.00 with N percent savings) — siel_log.parse_amzn_apex_price 로 ₹X 만 추출. fallback: 구 a-offscreen layout (변경 전 selector). variant carousel 회피 위해 corePriceDisplay scope [1] 한정'),
  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','main',d,'discount_type',
       './/*[contains(text(),"Hot Deal") or contains(text(),"Hot deal") or contains(text(),"Super Deals") or contains(text(),"Bank offer") or contains(text(),"Bank Offer") or contains(text(),"Limited") or contains(text(),"Big Saving") or contains(text(),"Coupon") or contains(text(),"Special") or contains(text(),"Deal of") or contains(text(),"Trending")]',
       NULL,
       '사용자 정책: Hot Deal/Hot deal (대소문자 양쪽) / Super Deals / Bank offer / Limited 등 모두 deal type. % off 는 savings 별도'),
