  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Amazon','main',d,'final_sku_price',
       './/span[@class="a-price" and @data-a-color="base"]//span[@class="a-offscreen"] | .//span[@aria-label="Currently unavailable."]/span | .//span[normalize-space(text())="Currently unavailable."]',
       './/span[contains(@class,"a-price") and not(@data-a-strike)]//span[@class="a-offscreen"]',
       '가격 매치 우선 (DOM 순서) — 가격 부재 시 aria-label="Currently unavailable." inner span 또는 text fullmatch 캡처. a-color-state class 는 outdated (Amazon DOM 변경). variant chooser ("See options") 카드는 valid null — detail 에서 채움'),
  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','main',d,'discount_type',
       './/*[contains(text(),"Hot deal") or contains(text(),"Limited") or contains(text(),"Big Saving") or contains(text(),"Coupon") or contains(text(),"Special") or contains(text(),"Deal of")]',
       NULL,
       'promotion 표시만 — Hot deal 우선. Bank Offer 는 결제 혜택이라 제외, % off 는 savings 별도'),
