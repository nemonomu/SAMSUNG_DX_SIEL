  ...
  FOREACH d IN ARRAY domains LOOP
    ...
    VALUES
      ...
      ('Flipkart','main',d,'discount_type',
       './/div[normalize-space(text())="Hot deal" or normalize-space(text())="Limited time deal" or normalize-space(text())="Big Saving" or normalize-space(text())="Big Saving Days" or normalize-space(text())="Top Deal"]',
       NULL,
       'deal badge — exact text 매치 (Bank Offer 결제 promo 는 상품 할인 아님 — 제외). % off 는 savings 에서 별도 수집'),
